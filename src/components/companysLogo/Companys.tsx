  ...
  return (
    <section className="py-6">
      <div className="relative overflow-hidden">
        <div className={`flex ${noRotate ? "" : direction === "left" ? "logos-slider-left" : "logos-slider-right"}`}>
          {logos.map((logo, index) => (
            <div key={index} className="mx-5 flex items-center justify-center min-w-[120px]">
              <Image
                src={logo.src}
                alt={logo.alt}
                width={150}
          ...
          {logos.map((logo, index) => (
            <div key={`dup-${index}`} className="mx-5 flex items-center justify-center min-w-[120px]">
              <Image
                src={logo.src}
                alt={logo.alt}
                width={150}

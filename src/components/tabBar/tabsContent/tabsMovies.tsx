...
const TMDB_API_KEY = process.env.NEXT_PUBLIC_TMDB_KEY || process.env.TMDB_KEY || "";
const TMDB_BASE_URL = process.env.NEXT_PUBLIC_TMDB_BASE_URL || "https://api.themoviedb.org/3";
const TMDB_IMAGE_BASE_URL = process.env.NEXT_PUBLIC_TMDB_IMAGE_BASE_URL || "https://image.tmdb.org/t/p/w1280";
...
const formatMovieData = (data: TMDBMovie[], mediaType: "movie" | "tv" = "movie"): Movie[] => {
  return data.map((item) => ({
    id: item.id.toString(),
    title: mediaType === "movie" ? item.title || "Unknown Title" : item.name || "Unknown Title",
    posterPath: item.poster_path ? `${TMDB_IMAGE_BASE_URL}${item.poster_path}` : "/movieImages/dummamig4.jpg",
    backdropPath: item.backdrop_path ? `${TMDB_IMAGE_BASE_URL}${item.backdrop_path}` : "/movieImages/dummamig4.jpg",
  ...
  useEffect(() => {
    const fetchMovies = async () => {
      ...
      try {
        ...
        const formattedData = formatMovieData(data.results.slice(0, 6), mediaType);
